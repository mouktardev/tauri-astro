...
// const items = ["asset://localhost/%2Fhome%2Fmouktar%2Fimages%2Ffreysteinn-g-jonsson-LI03w3L-PxU-unsplash.jpg", "asset://localhost/%2Fhome%2Fmouktar%2Fimages%2Fjayson-hinrichsen-AlK8-7FU_-E-unsplash.jpg", "asset://localhost/%2Fhome%2Fmouktar%2Fimages%2Fkai-wenzel-06MHFfYv6YY-unsplash.jpg"]
function FileListButton() {
	...
	return (
		<section className="container mx-auto p-5 w-50 h-50">
			<Button
				className="border border-neutral-800 dark:border-white text-lg dark:text-white p-4"
				onClick={chooseFiles}
			>
				Choose images
			</Button>
			<ul className="grid lg:grid-cols-3 sm:grid-cols-1 gap-4 mt-5">
				{Images.length > 0 ? (
					Images.map((image, index) => (
						<li
							className="min-w-[300px] backdrop-blur-md bg-white/10 dark:bg-black/30 shadow-3xl dark:shadow-none overflow-hidden rounded-lg"
							key={index}
						>
							<Image className="w-full h-60 object-cover" src={image} alt="" />
							<div className="flex gap-5 justify-between p-3">
								<p className="text-sm text-neutral-900 dark:text-white">
									{image.substring(image.lastIndexOf(`%2`) + 2)}
								</p>
								<Dots className="h-6 w-6 text-neutral-800 dark:text-white" />
							</div>
						</li>
					))
				) : (
